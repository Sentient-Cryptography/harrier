...
static CBigNum bnProofOfWorkLimit = CBigNum().SetCompact(504365644); // Harrier: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Harrier Signed Message:\n";
...
{
    // Harrier: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Harrier: 840k blocks in ~4 years
...
static const int64 nTargetTimespan = 302400; // Harrier: 3.5 days
static const int64 nTargetSpacing = 120; // Harrier: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Harrier: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Harrier: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Harrier: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "BBC News - 16/10/2017 - Somalia attack: Death toll rises in Mogadishu blast";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 5000000000;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("2979026d62501b0ab68fef1ab9eb285337641de64c66b20782e32474b196fe8aac3e367dd413aeced2bbdecfd95bb0270cdca26a10d3eb4e56e02bcbb4e47b28c2") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1508150516;
...
unsigned char pchMessageStart[4] = { 0x1b, 0xc5, 0xd9, 0xf2 }; // Harrier: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HarrierMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HarrierMiner : ProcessBlock, block not accepted");
    }
